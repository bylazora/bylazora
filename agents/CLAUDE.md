...
- Draft, then run, then read the verdict. Use the MCP tools: bench (cpu, gpu,
  cuda) and validate.
- Connect through MCP: `npx bylazora setup claude` writes the client
  config, and `bylazora mcp` serves the tools (the same server is
  `bylazora-core mcp` directly). The CLI delegates; it never marks a run
  proven.
...
- Run `bylazora-core migrate verify <job>` (or `bylazora verify` from
  inside the workspace; it finds the job via SPEC.json) to build, run and
  gate. The verdict record lands in runs/ and is never overwritten, which is
  what makes the trail an audit artefact.
- A MISMATCH is a diff to fix, never a threshold to tune.
